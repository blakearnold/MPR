						...
						int polarity, int shareable)
{
	...
	if (i >= PNP_MAX_IRQ && !warned) {
		printk(KERN_WARN "pnpacpi: exceeded the max number of IRQ "
				"resources: %d \n", PNP_MAX_IRQ);
		warned = 1;
		return;
	}
						...
						int bus_master, int transfer)
{
	...
	if (i < PNP_MAX_DMA) {
		...
		printk(KERN_WARN "pnpacpi: exceeded the max number of DMA "
				"resources: %d \n", PNP_MAX_DMA);
		warned = 1;
	}
}
					       ...
					       u64 io, u64 len, int io_decode)
{
	...
	if (i < PNP_MAX_PORT) {
		...
		printk(KERN_WARN "pnpacpi: exceeded the max number of IO "
				"resources: %d \n", PNP_MAX_PORT);
		warned = 1;
	}
}
						...
						int write_protect)
{
	...
	if (i < PNP_MAX_MEM) {
		...
		printk(KERN_WARN "pnpacpi: exceeded the max number of mem "
				"resources: %d\n", PNP_MAX_MEM);
		warned = 1;
	}
}
